...
/*
 * 컬렉션 이라고 불리우는 객체들에 대해서 알아보자.
 *  - (기본데이터가 아닌) 객체관리를 용이하기 위해서 설계된 객체들이다.
 *  - 배열로 처리하는 것보다는 훨씬 쉽고 간단하게 프로그램을 만들어낼수 있다.
 *  
 *  - 컬렉션형 객체들은 종류가 다양하고, 작동하는 법도 약간씩은 다르다.
...
public class Source01_Collections {
	public static void main(String[] args) {
		...
		//=====================================================
		// army 반복문 돌려서 s 객체가 army 에 저장되어있는지, 아닌지 탐색
		boolean find = false;
		for(int i=0; i<army.length; i++) {
			if(army[i] == s) {
				find = true;
		...
		// 포함여부나, 중복확인, 남아있는 공간체크같은 것들을 일일이 각 칸을 확인해보면서
		// 관리를 해나갈수 밖에 없다.
